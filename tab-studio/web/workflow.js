...
var Workflow = (function () {
  'use strict';
  // The backend is the local FastAPI server, same-origin (it serves this page).
  var API = '/api';
  ...
  function health() {
    ...
    (function tryNext(i) {
      if (i >= bases.length) {
        ...
        setPipelineButtons();
        setText('backendHint', 'Editor, MIDI and tabs work offline. For stems / transcription / YouTube / saving projects start the backend: cd tab-studio/server && .venv\\Scripts\\python -m uvicorn app:app --port 8000');
        return;
      }
