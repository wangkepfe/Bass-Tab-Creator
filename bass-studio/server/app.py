...
Static:
  /studio  -> the editor   (bass-studio/web)
  /        -> repo root     (serves the unmodified Bass Tab Creator + shared assets)
...
Run:  cd bass-studio/server  &&  uvicorn app:app --port 8000
"""
import os
import re
import sys
...
ASSETS_DIR = ROOT_DIR / "assets"                    # bundled example MIDIs
PROJECTS_DIR = ROOT_DIR / "projects"                # saved projects (git-ignored) — the app's "database"
PROJECTS_DIR.mkdir(parents=True, exist_ok=True)
WORK_DIR = Path(tempfile.gettempdir()) / "bass-studio-jobs"
WORK_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="Bass Studio")
# The SPA is served same-origin by this app and the API is unauthenticated and now
# persists projects/audio to disk — so DON'T allow arbitrary origins (a wildcard
# would let any visited website read/overwrite the local projects). Allow only the
# loopback origins the app is actually served from.
app.add_middleware(
    CORSMiddleware,
...
async def create_job(
    ...
    job = _new_job()
    in_path = Path(job["dir"]) / ("input" + Path(file.filename or "audio.wav").suffix.lower())
...
# -----------------------------------------------------------------------------
# static mounts (LAST). The unified app (web/) is served at the root; the repo
# /assets folder is exposed for the built-in example MIDI.
# -----------------------------------------------------------------------------
